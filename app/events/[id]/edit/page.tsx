...
export default function EditEventPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (!eventId || status === 'loading' || status === 'unauthenticated') return;
  ...
  if (status === 'loading' || loadingEvent) {
    return (
      <div style={pageStyle}>
        <h1 style={headingStyle}>Edit Event</h1>
    ...
    return (
      <div style={pageStyle}>
        <div style={accessDeniedStyle}>
          <p style={{ fontSize: '1.1rem', marginBottom: '12px' }}>Event Not Found</p>
          <Link href="/events" style={{ color: '#00693f', fontWeight: 'bold' }}>
            Back to Calendar
          </Link>
        </div>
      </div>
      ...
      <div style={pageStyle}>
        <div style={accessDeniedStyle}>
          ...
          <p style={{ fontSize: '0.95rem' }}>
            You need the <strong>calendar</strong> role to edit events.
          </p>
